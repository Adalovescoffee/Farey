...
using Luxor, PlutoUI, Colors, SimpleWeightedGraphs, Graphs

# ╔═╡ fe2fd530-bb41-458c-bd8a-83074e5b36a7
TableOfContents()

# ╔═╡ 15f0e8a0-59e4-11ef-288b-17fd5d6af339

md"# Generalized Farey Diagrams 
by Ada
## 0.Introduction 
(Coming Soon)
## 1. The Farey Diagram
The goal of this project is to study numbers inside a new geometry other than a simple line, mainly rational numbers as any real number can be defined as a limit of rational numbers (We say that $\mathbb{Q}$ is dense in $\mathbb{R}$), here instead we work on a projectively extended real line ie taking the usual regular number lane and glueing the edges ($+\infty$ and $-\infty$ in one point ie all divergent series converges to infinity)
"


# ╔═╡ b47b73e6-7b7f-41eb-8440-71162185fbe2
md"
There are different way to map the number line depending on which property you do want to keep, but for now we'll focus on the Farey Series and how it maps to the entire set of rational numbers as seen in the picture below 
"

# ╔═╡ dfc9d630-f7bd-4b10-adc0-70659c5b11c4
md"""
!!! info "Definition 1.1: Rational Numbers"

	 ${\text{Let }(a,b)  \in \mathbb{Q} \text{ such that } gcd(a,b) = 1}$
	$\text{then the fraction } q = \frac{a}{b} \text{ is in its irreductible form  }$
"""

# ╔═╡ 21144191-0199-44c4-8140-17b26f00934b
md"### 1.1.Farey Series :" 

# ╔═╡ 513dbd85-0a5b-4e2e-b294-8cb2c835a81e
md"""
!!! info "Definition 1.2: Mediant operator"

	${\text{Let } (p,q) = (\frac{a}{b},\frac{c}{d}) \in \mathbb{Q}^2, \text{we define the mediant as the function :} }$ 
	$M:\mathbb{Q}^2 \rightarrow \mathbb{Q}$
	$(\frac{a}{b}, \frac{c}{d}) \rightarrow \frac{a+c}{b+d}$
	
"""

# ╔═╡ 89f03602-52c6-4dd4-aa2c-a2722d18ecb9
function Mediant(p,q)
	return (numerator(p)+ numerator(q))//(denominator(p)+denominator(q))
end

# ╔═╡ 1ce08119-e77b-406e-a923-0d2ceb66547c
md""" 
With this definition we can define our farey sequence by iterating the mediant over a set of rational numbers !
note : For the sake of simplicity and norms etc... we decide to start with 2 numbers (applying the mediant over the same number is irrelevant) **$\frac{0}{1}$** and **$\frac{1}{0}$** 
!!! info "Definition 1.3: Farey Sequence"
	We define the farey diagram as $M^n(F_0)$:

	$F_0 = (\frac{1}{0},\frac{0}{1})$
	
	$F_1 = (\frac{1}{0}, \frac{1}{1}, \frac{0}{1})$

	$F_2 = (\frac{0}{1}, \frac{1}{2}, \frac{1}{1},\frac{2}{1}, \frac{1}{0})$

	$...$
"""

# ╔═╡ 2a63d873-6ca4-4cb3-8674-9dec28533f3a
@bind M PlutoUI.Slider(1:10)

# ╔═╡ 4cf5341d-bc0e-47b7-9cd9-1873327cb237
function farey_series(n::Int)
    farey = [0//1, 1//1]
    
    # Perform n iterations
    for _ in 1:n
        new_farey = []
        for i in 1:length(farey)-1
            push!(new_farey, farey[i])
            mediant = Mediant(farey[i], farey[i+1])
            push!(new_farey, mediant)
        end
        push!(new_farey, farey[end])
        farey = new_farey
    end
    return farey
end
...
# ╔═╡ b9140909-8f10-45c4-8561-cf0da6d9a233
farey_series(M)

# ╔═╡ 6cfce3c1-7c93-44f0-a4d0-88d075116e1d
md"At first it looks like a wrong type of sum of fraction, but this equation holds a lot of very cool properties that we'll build on, we can even consider it as a graph, connect every parent fraction with its child! "

# ╔═╡ a29492eb-68c0-46f4-a794-6f11986b7a59
md"""
!!! danger "Note" 
	One would say that this sequences would only have results in $\mathbb{R}$ and you'd be right! For us to attempt to map the whole real number set we'd have to also have the same operation work but starting with $(\frac{-1}{0},\frac{0}{1})$ like the diagram!! 
"""

# ╔═╡ 342d8ba6-e0e0-491f-8982-4c87cd11bf4a
md" Just out of curiosity (and for fun), let's think abt the properties of this graph:  

"

# ╔═╡ ef3a3b0c-cb05-4a75-b561-7f3dab9b8701
md"#### 1.1.1.Graph Theory detour 
- Every iteration, the number of vertices is doubled, therefore $V_{F_n} = 2V_{F_{n-1}}$. 
Since $V_0 = 2: V_{F_n} = 2^{n+1}$  
- Since every new vertex is connected to 2 old vertices, we can conclude that :$E_{F_{n}} = \sum_{k = 0}^{n+1}2^k -2 = \sum_{k = 1}^{n+1}2^k -1 = 2\sum_{k = 0}^{n}2^k-1 = 2^{n+2} - 3$
- Using Euler's formula on graphs $V - E + F = 2$, the number of faces is 
$F = 2 + 2^{n+2} - 3 - 2^{n+1} = 2^{n+1}-1$ which is wrong the actual value is $idk i forgot i'm too lazy for now$

"

# ╔═╡ d3109bfb-33e7-4682-8278-9e9e5a69d722
md"""
!!! tips "Property 1.1: "
	Let $(a,b),(c,d) \in \mathbb{Z}:$
	$\frac{a}{b}< \frac{a+c}{b+d} = M(\frac{a}{b}, \frac{c}{d}) < \frac{c}{d}$
"""

# ╔═╡ d6e37a2f-0305-438e-b218-0eb36e9c2552
md"##### Proof:

Let $(a,b),(c,d) \in \mathbb{Z} :a(b+d) - b(a+c) = ab+ad -ab-bc = ad - bc$

Since $\frac{a}{b} < \frac{c}{d}: ad - bc > 0$ therefore $a(b+c) - b(a+c) > 0$ which implies that $\frac{a}{b}< \frac{a+c}{b+d}$ 

QED!
"

# ╔═╡ 2d7118de-ff94-487b-a63f-3ac1af1db35e
md"

"

# ╔═╡ 2c6f2a9e-ef83-4fe1-9e42-58e592293866
md"
Now one would wonder, yes this is interesting, but does it map to the entire set of rational numbers and the answer is Yes :D 
But that requires some more thinking, for now let's think about the fundamental theorem, the upcoming theorem tho is a very interesting theorem and happens to be the main reason why I even decided to go on this journey. 
"

# ╔═╡ a11af13e-26dd-4bf6-bbde-506f469388ed
md"""
!!! tips "Property 1.2: THE Theorem"
	Let $(p = \frac{a}{b}, q = \frac{c}{d}) \in \mathbb{Q}$ there is an edge between $\frac{a}{b}$ and $\frac{c}{d}$ if and only if $det(\begin{bmatrix} a & c \\b & d \end{bmatrix}) = \pm 1$
"""

# ╔═╡ cf2b3e53-5389-4a11-8783-80599e195f22
md" #### Proof
##### Proving the direct implication:
- For n = 1 : $det(\begin{bmatrix} \pm1 & 0 \\0 & 1 \end{bmatrix}) = \pm 1$
- Suppose the property is true for every 3 consecutive numbers in the farey series, let's prove that it's true for next farey series:
Let $\frac{a}{b}, \frac{c}{d} \in F_n$ we know by induction that $det(\begin{bmatrix} a & c \\b & d \end{bmatrix}) = \pm 1$, in F_n+1 , we'd have $\frac{a}{b}$ and $\frac{c}{d}$ connected to $\frac{a+c}{b+d}$, we need then to prove that : 

-$det(\begin{bmatrix} a & a+c \\b & b+d \end{bmatrix}) = \pm 1$: $a(b+d)-b(a+c) = ab -ba + ad -bc = \pm 1$       $QED_1$ 

-$det(\begin{bmatrix} a+c & c \\b+d & d \end{bmatrix}) = \pm 1$: $(a+c)d - (b+d)c = ad+cd -bc -dc =ad -bc = \pm 1$ $QED_2$
By Godel's 4th law of completeness, QED 
"

# ╔═╡ 2454ddac-86ae-4b54-9189-1398cb367571
md"
##### Proving the converse (ie : if $ad-bc = 1$ (the other cases are the same) then there is an edge joining $\frac{a}{b}$ to $\frac{c}{d}$)
For this we need to do some reverse engineering: 
- We know that if we have 2 fractions $\frac{p}{q}$,$\frac{w}{t}$ we can operate the mediant to get a new vertex that will connect to the two existing values. We can deduce that if we have two fractions 
- Thus if we have two connected fractions, all we have to do is to first compare them to know their relative position, and apply $M_T(\frac{a}{b},\frac{c}{d}) = \frac{c-a}{b-d}$, and we repeat this process until we get an element from $F_1$, we have thus proven the converse because we can find a path from $F_1$ to that element! :D
**IMPORTANT** : since $ad-bc = \pm 1$ that implies by Bezout's theorem we can conclude that $gcd(a,b) = gcd(c,d) = 1$ thus, this algorithm always gives the fraction in its lowest terms!!

TODO: Have an animation or smt showing this 
"

# ╔═╡ 79531990-2aa2-41f6-9cfc-fac9ee3a661f
md"""
!!! danger "Note"
	This algorithm shown is basically the continued fraction decomposition of our fraction, with the following theorem we can finally prove that knowing the position of a fraction in the diagram is equivalent to knowing its continued fraction 
"""
...
# ╔═╡ 47591f52-c5f5-48ca-b72b-becff0adedb4
md"
With this we can finally prove the following important theorem

"

# ╔═╡ 53a6d474-c8c1-43ca-9c47-f1990ca501f0
md"""
!!! tips "Property 1.3: "
	Every fraction $\frac{a}{b}$ occurs at some point in the farey diagram.
"""

# ╔═╡ d51d12dc-300b-450b-8138-39e2682cbbd6
md" #### Proof 
- You only need to realize that since $\frac{a}{b}$ is in its lowest terms, $gcd(a,b) = 1$, by Bezout's theorem : $\exists (u,v)\in \mathbb{Z}: au + bv = 1$, trivial by P1.1


"

# ╔═╡ 30fc3693-c1c1-4f4a-8681-0bd56adfc0eb
md"
In the next section, we try to define new rules for when the $det(ad-bc)=\pm 2$ and prove them! 
"

# ╔═╡ 37210c19-22da-4fc2-804e-335884ed79a6
md"
### 1.2.Journey into the farey multiverse "

# ╔═╡ 76690aed-72cf-48a2-b906-149f71c96a9f
...
# ╔═╡ 6b793c34-b68b-472b-9fe3-801afdde1a28
function equiprojection(m::Int,G)
    @svg begin 
    Drawing(1000, 1000)
    Luxor.origin()
    background("#1f1f1f")
	...
	for (k, pts) in DictG
        ...
        circle(Luxor.Point(coord_x, coord_y), 1, :fill)
...
# ╔═╡ 4938026f-6af5-493d-b29a-005d1da00101
plot_farey_graph_with_edges2equi(5,FareyGraph(5))

# ╔═╡ e27908e5-3e41-4728-aded-0bbf68da7216
...
# ╔═╡ 1dfcf03a-9301-43a0-8239-6d54dcd35a84
md"""### References
[Wiki Stern Brocot_tree](https://en.wikipedia.org/wiki/Stern%E2%80%93Brocot_tree)

[Alan Hatcher's book](https://pi.math.cornell.edu/~hatcher/TN/TNpage.html)
...
manifest_format = "2.0"
project_hash = "904d978207beed184f4dfcf03a00fbb264ff3e43"
